...
func TestDevOnCloud(t *testing.T) {
	url := "http://auth.eensymachines.in/devices/000000007920365b"
	status := &DeviceStatus{}
	if err := DeviceStatusOnCloud(url, status); err != nil {
		t.Error(err)
	}
	t.Log(status)
	if status.Lock {
		t.Log("Device is locked")
		return
	}
	t.Log("Device is unlocked")
	return
}
